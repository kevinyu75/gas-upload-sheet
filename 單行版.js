 ...
 **************************************************************/
function 生成Line推播文案_ActiveRow(mode = "multi") {
  ...
  try {
    ...
    let text = "";
    if (mode === "multi") {
      text = 生成Line推播文案_OpenAI(name, desc, approxLen, "multi");
      text = 加入智能Emoji_(text, name, desc); // 第一行加 emoji
      text = 清理文案_(text);
      text = 隨機變化文案_(text); // 第二、三行隨機化
    } else if (mode === "single") {
      text = 生成Line推播文案_OpenAI(name, desc, approxLen, "single");
      text = 清理文案_(text);
    }
 ...
 * OpenAI：多行版 / 單行版 共用
 ******************************************************************/
function 生成Line推播文案_OpenAI(name, desc, approxLen, mode) {
  const props  = PropertiesService.getScriptProperties();
...
`;
    ...
    prompt = `
你是中文行銷文案助手，專門生成 LINE 社群推播文字。

請根據以下商品資訊，生成一則 **單行文案**：
- 必須完整包含「品名要素」：${name}（不可簡化或刪減）
- 長度約 ${approxLen} 字（建議落在 ${minLen}-${maxLen} 字）
- 開頭加一個 emoji 吸睛
- 單行輸出，不可換行
- 品名後加上 1 個亮點描述（口感/情境/特色）
- 不要出現效期、保存方式、重量、出貨週期等物流資訊
...
function 加入智能Emoji_(txt, name, desc) {
  ...
  let lines = txt.split('\n');
  if (lines.length > 0) {
    lines[0] = lines[0].replace(/^[\u{1F300}-\u{1FAFF}]\s*/u, ''); // 移除 AI 自帶 emoji
